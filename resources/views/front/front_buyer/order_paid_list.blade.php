            ...
            <div class="col-md-6 text-right">
                ...
                <div class="display-inline ml-20 top-align">
                    ...
                    <p style = "text-align: right;font-size: 10px;">
                        ...
                        |
                        @if(!$order->isSavedSeller())
                            <a href = "javascript:void(0)" class = "color-blue" data-seller-id = "{{$order['seller_id']}}" data-user-id = "{{$order['user_id']}}" onclick = "onSaveSeller(this)">saveSeller</a>
                        @else
                            <a href = "javascript:void(0)" class = "color-blue" >isSavedSeller</a>
                        @endif
                    </p>
                    @endif
                </div>
                <div class="display-inline ml-20 top-align">
                    <div class="row ml-0 mr-0">
                        @if($order['state']*1 == 2)
                        <button type = "button" class="visit-btn" data-order-id = "{{$order['id']}}" data-order-state = "3" onclick = "updateOrderState(this)">Check</button>
                        @endif
                        @if($order['state']*1 == 3)
                            <button type = "button" class="visit-btn" data-order-id = "{{$order['id']}}" onclick = "onEvaluateOrder(this)">Evaluate</button>
                        @endif
                    </div>
                </div>
            </div>
...
<div class = "text-right" style = "margin-top:10px;">
    <?php $searchFun = "searchPaidOrderList";?>
